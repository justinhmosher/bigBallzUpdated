...
def activate(request, uidb64, token):
	...
	if myuser is not None and generate_token.check_token(myuser,token):
		...
		try:
			...
			NW_promo.save()
			NW_paid = PaidBL(username = myuser.username)
			NW_paid.save()
			NW_promo = PromoUserBL(username = myuser.username)
			NW_promo.save()
			NW_paid = PaidBS(username = myuser.username)
			NW_paid.save()
			NW_promo = PromoUserBS(username = myuser.username)
			NW_promo.save()
